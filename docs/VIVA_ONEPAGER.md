...
- **Q: Why RandomForest, not XGBoost/CatBoost?** *We ran the comparison (roadmap M2, `compare_models.py`) on the same split — and honestly, RF is **not** the best: LightGBM/CatBoost score higher (AUC 0.876–0.878 vs 0.850), calibrate better, and are ~15–35× smaller and ~30× faster (CatBoost 0.34 MB / 1.7 ms). RF was the initial pick and is retained in this build for continuity; the evidence-backed recommendation is to swap the estimator to CatBoost while keeping the single-pipeline architecture. Surfacing that honestly is the point of the comparison.*
