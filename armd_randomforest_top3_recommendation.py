# -*- coding: utf-8 -*-
"""armd_randomforest_optimized_split_testing.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1LeEVuzhZ_uc_hsKpW6DSAa8_uptFcXaq

# ARMD RandomForest Top-3 Antibiotic Recommender (Colab, RAM-efficient)

This notebook trains a **RandomForestClassifier** on the **ARMD** dataset using the feature set from your approved PDF:
- core cohort: `culture_description`, `was_positive`, `organism`, `antibiotic`, `susceptibility`
- demographics: `age`, `gender`
- labs: `wbc_median`, `cr_median`, `lactate_median`, `procalcitonin_median`
- prior antibiotic classes
- prior infecting organisms
- ward flags (`ICU`, `ER`, `IP`) optional

It is written to be **free-Colab friendly**:
- reads only needed columns
- filters to your selected antibiotics **before** large joins
- processes big files in **chunks**
- limits prior-organism one-hot width with `MAX_PRIOR_ORGS`
- avoids huge sparse/dense explosions
"""

# If needed in a fresh Colab runtime, uncomment:
...
# CONFIG
# =========================
# Put the ARMD CSV files inside this folder in your Drive.
BASE_DIR = '/content/drive/MyDrive/'

# Free-Colab friendly knobs
CHUNK_SIZE = 150_000      # reduce to 100_000 if RAM is still high
MAX_PRIOR_ORGS = 50       # reduce to 25 if RAM is still high
N_ESTIMATORS = 300        # reduce to 180 if training is slow
MAX_DEPTH = 18
MIN_SAMPLES_LEAF = 3
TEST_SIZE = 0.20
VALID_SIZE_FROM_TRAIN = 0.15   # threshold tuning set from training data
RANDOM_STATE = 42

# Feature sparsity controls
TOP_N_ORGANISMS = 40       # low-frequency organisms become "other"
TOP_N_CULTURE_SITES = 25   # low-frequency culture sites become "other"
MAX_BINARY_FEATURES = 120  # keeps most frequent prior_abx/prior_org/ward binary features

# Threshold tuning goal:
# For antibiotic recommendation, we usually care about not missing susceptible options.
# Use recall_first to reduce false negatives; use f1 if examiner asks for balanced score.
THRESHOLD_POLICY = 'recall_first'   # options: 'recall_first', 'f1'
...
REQUIRED_FILES = [
    ...
    'microbiology_cultures_ward_info.csv',  # optional
]
...
def map_susceptibility(value):
    ...
    # Binary target for cleaner model:
    # susceptible -> 1
    # resistant -> 0
    # intermediate / unknown -> dropped
...
def choose_existing_columns(path, wanted_cols):
...
def optimize_binary_columns(df):
    for col in df.columns:
        if col.startswith('prior_abxclass__') or col.startswith('prior_org__') or col.startswith('ward__'):
...
missing_required = [
    fname for fname in REQUIRED_FILES[:-1]  # ward is optional
...
print("USE_WARD =", USE_WARD)

# =========================
# 1) LOAD & FILTER CORE COHORT FIRST
# =========================
...
# Standardize antibiotic names and keep only overlap antibiotics
...
# Keep only the columns we actually need from this point
...
# Keep only anon_ids we actually need.
cohort_ids = set(core['anon_id'].astype(str).unique())
print(f"Unique anon_id count after core filtering: {len(cohort_ids):,}")
...
# Convert age buckets like "35-44 years" into numeric midpoint
def convert_age_to_numeric(x):
    ...
    # fallback: try extracting range automatically
    import re
...
# =========================
# 4) PRIOR ANTIBIOTIC CLASS EXPOSURE (CHUNKED)
# =========================
# Goal: multi-hot features per anon_id, but only from rows tied to filtered cohort_ids.
...
print("Unique antibiotic classes found:", len(all_abx_classes))
...
# =========================
# 5) PRIOR INFECTING ORGANISMS (CHUNKED, WIDTH-LIMITED)
# =========================
...
# Try a few possible column names because dataset versions sometimes differ slightly.
possible_prior_org_cols = [
    'prior_organism',
...
if prior_anon_col is None or prior_org_col is None:
    raise ValueError(
        f"Expected anon_id and one of {possible_prior_org_cols} in prior organism file. Header has: {prior_header[:25]}"
    )
...
top_prior_orgs = [k for k, _ in sorted(prior_freq.items(), key=lambda x: x[1], reverse=True)[:MAX_PRIOR_ORGS]]
print("Top prior organisms retained:", len(top_prior_orgs))
print(top_prior_orgs[:15])
...
if USE_WARD:
    ...
    # Keep ICU, ER, IP. OP is redundant if IP exists.
    rename_map = {
        'hosp_ward_ICU': 'ward__icu',
        'hosp_ward_ER': 'ward__er',
        'hosp_ward_IP': 'ward__ip',
    }
...
# Optional: remove exact duplicate feature-target rows
...
# =========================
# 8) SPARSITY REDUCTION + FEATURE SET
# =========================
# This improves RandomForest generalization and keeps Colab RAM usage controlled.
# Rare organism/culture categories create noisy splits, so we group them as "other".
...
if 'culture_description' in df.columns:
    ...
    df['culture_description'] = df['culture_description'].where(df['culture_description'].isin(top_cultures), 'other')
...
binary_cols = [c for c in X.columns if c.startswith('prior_abxclass__') or c.startswith('prior_org__') or c.startswith('ward__')]
...
# =========================
# 9) STRATIFIED TRAIN / VALIDATION / TEST SPLIT + RANDOM FOREST
# =========================
# Test set is kept untouched for final reporting.
# Validation set is used only to choose the best decision threshold.
...
# =========================
# 10) PROPER SPLIT TESTING + THRESHOLD TUNING
# =========================
# Class meaning:
# 0 = Resistant
# 1 = Susceptible
...
y_val_pred_050 = evaluate_binary_classifier(y_val, val_prob, threshold=0.50, title='VALIDATION @ default 0.50')
...
display(threshold_df.sort_values(['f1_1', 'balanced_accuracy'], ascending=False).head(10))
print(f"Selected BEST_THRESHOLD = {BEST_THRESHOLD:.3f} using policy = {THRESHOLD_POLICY}")

_ = evaluate_binary_classifier(y_test, test_prob, threshold=0.50, title='TEST @ default 0.50')
...
print("\n===== REPORT SUMMARY =====")
display(split_test_summary)

# =========================
# 13) TOP-3 RECOMMENDATION FUNCTION
# =========================
def recommend_top3(patient_input, model, candidate_antibiotics, threshold=None):
    """
    Scores each candidate antibiotic for one patient/culture case.
    Returns:
    - top3: top 3 antibiotics by P(Susceptible)
    - all_scores: full ranked table
    """
    rows = []
    ...
    result = pd.DataFrame({
        ...
        'prediction_at_best_threshold': (probs >= BEST_THRESHOLD).astype(int)
...
# =========================
# 10B) TOP-3 STYLE SPLIT TESTING
# =========================
# This evaluates recommendation behavior on the held-out test set.
# For each patient/culture context, we rank the 32 selected antibiotics and check
# whether the actual susceptible antibiotic appears in the Top-3.
# To keep this Colab-friendly, we sample contexts from the test set.

TOP3_EVAL_CONTEXTS = 500   # increase to 2000 if runtime/RAM is okay
...
pos_contexts = (
    test_eval[test_eval['actual_target'] == 1]
    .drop_duplicates(subset=context_cols)
)
...
if len(pos_contexts) == 0:
    ...
    sample_contexts = pos_contexts.sample(
        n=min(TOP3_EVAL_CONTEXTS, len(pos_contexts)),
        random_state=RANDOM_STATE
    )
    ...
    for _, ctx_row in sample_contexts.iterrows():
        ...
        for c in context_cols:
            mask &= (test_eval[c].values == ctx[c])
        actual_sus_abx = set(test_eval.loc[mask & (test_eval['actual_target'] == 1), 'antibiotic'].astype(str).tolist())

        top3, all_scores = recommend_top3(ctx, model, SELECTED_ANTIBIOTICS, threshold=None)
    ...
    top3_hit_rate = np.mean(hits)
    mrr = np.mean(reciprocal_ranks)

    print("===== TOP-3 RECOMMENDATION TESTING =====")
    print("Sampled contexts:", len(sample_contexts))
    print("Top-3 Hit Rate:", round(top3_hit_rate, 4))
    print("Mean Reciprocal Rank:", round(mrr, 4))

# =========================
# 11) FEATURE IMPORTANCE
# =========================
# With OrdinalEncoder, each categorical column remains one transformed feature.
# This is useful for FYP explanation: which input groups affected susceptibility most?
...
# 12) SAVE ARTIFACTS
# =========================
ARTIFACT_DIR = os.path.join(BASE_DIR, 'armd_model_artifacts')
os.makedirs(ARTIFACT_DIR, exist_ok=True)

joblib.dump(model, os.path.join(ARTIFACT_DIR, 'rf_top3_recommender_optimized.joblib'))
joblib.dump(feature_cols, os.path.join(ARTIFACT_DIR, 'feature_cols.joblib'))
joblib.dump(SELECTED_ANTIBIOTICS, os.path.join(ARTIFACT_DIR, 'selected_antibiotics.joblib'))
joblib.dump(importances, os.path.join(ARTIFACT_DIR, 'feature_importances.joblib'))
joblib.dump(BEST_THRESHOLD, os.path.join(ARTIFACT_DIR, 'best_threshold.joblib'))
joblib.dump(split_test_summary, os.path.join(ARTIFACT_DIR, 'split_test_summary.joblib'))
...
with open(os.path.join(ARTIFACT_DIR, 'metadata_optimized.json'), 'w') as f:
...
# =========================
# 15) OPTIONAL: QUICK SANITY CHECK FOR UNSEEN CATEGORY HANDLING
# =========================
# The model should still run even if some categories were not seen in training.
unseen_patient = {
...
top3_unseen, _ = recommend_top3(unseen_patient, model, SELECTED_ANTIBIOTICS)
display(top3_unseen)

"""## Notes

1. This optimized notebook uses **stratified train/validation/test split**:
   - training set trains the model
   - validation set tunes the decision threshold
   - test set is untouched until final evaluation

2. Target meaning:
   - `Susceptible` → `1`
   - `Resistant` → `0`
   - `Intermediate` is dropped for a cleaner first version.

3. Improvements included:
   - class imbalance handling using `class_weight='balanced_subsample'`
   - threshold tuning to reduce missed susceptible antibiotics
   - rare organism/culture grouping into `other`
   - width control for binary prior-history features
   - Top-3 recommendation split testing

4. If free Colab still gets close to RAM limit:
   - reduce `CHUNK_SIZE` to `100_000`
   - reduce `MAX_PRIOR_ORGS` to `25`
   - reduce `MAX_BINARY_FEATURES` to `80`
   - reduce `N_ESTIMATORS` to `180`
   - temporarily skip ward features
